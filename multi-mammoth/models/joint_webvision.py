...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--use_scheduler', type=int, choices=[0, 1], default=0, help='Use a scheduler')
    return parser
    ...
    def end_task(self, dataset):
        if dataset.SETTING != 'domain-il':
            ...
            if self.args.use_scheduler:
                self.scheduler = CosineAnnealingLR(self.opt, self.args.n_epochs)

            if self.args.loss_function == 'bce':
                self.loss = torch.nn.BCEWithLogitsLoss()
            elif self.args.loss_function == 'asymmetric':
                self.loss = AsymmetricLoss(gamma_neg=self.args.gamma_neg, gamma_pos=self.args.gamma_pos)
            else:
