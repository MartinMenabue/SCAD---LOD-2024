...
import timm
from models.scad_utils.vision_transformer import vit_base_patch16_224_twf
from models.scad_utils.vit_afd import TokenizedDistillation
import numpy as np
import os
import pickle
import sys
...
from utils.loss_functions import AsymmetricLoss
...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--adapter_type', default='mixer', choices=['mixer', 'channel', 'double_mixer', 'double_convit', 'mock', 'mimicking', 'twf_original', 'transformer',
                                                                 'clip_cross_attention', 'transformer_pretrained', 'transformer_pretrained_proj',
...
class SCAD(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        # loss = self.args.loss_w * self.loss(filtered_output, filtered_target.float())
        loss = self.args.loss_w * self.loss(output, labels[:, :offset_2].float())
        ...
        if len(self.buffer) == 0:
            ...
            loss_er = self.args.loss_w * self.loss(buf_outputs[:, :offset_2], buf_labels[:, :offset_2].float())
