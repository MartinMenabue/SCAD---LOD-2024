...
class PRS_mlab(rsvrBase):
    ...
    def __init__(self, args, device):
        ...
        self.rsvr = dict()
        self.rsvr_total_size = args.reservoir_size
        self.q = args.q_poa
    ...
    def _slab_to_onehot(self, ys):
        ...
        """
        y_onehot = torch.zeros(ys.size(0), self.nb_classes, dtype=torch.long, device=self.device)
    ...
    def _idxlist_to_multihot(self, idcs):
        ...
        """
        multihot = torch.zeros(self.nb_classes, dtype=torch.long, device=self.device)
        multihot[idcs] = 1
        return multihot
