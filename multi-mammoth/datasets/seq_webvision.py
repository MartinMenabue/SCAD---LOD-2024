...
class SequentialWebVision(ContinualDataset):
    ...
    N_TASKS = 15
    N_CLASSES = 750
    MEANS = (0.4569, 0.4354, 0.3904)
    STDS = (0.2737, 0.2664, 0.2791)
    # MEANS = (0.0, 0.0, 0.0)
    # STDS = (1.0, 1.0, 1.0)
    ...
    def get_normalization_transform():
        transform = transforms.Normalize((0.4569, 0.4354, 0.3904),
                                         (0.2737, 0.2664, 0.2791))
        # transform = transforms.Normalize((0.0, 0.0, 0.0),
        #                                  (1.0, 1.0, 1.0))
        return transform
    ...
    def get_denormalization_transform():
        transform = DeNormalize((0.4569, 0.4354, 0.3904),
                                (0.2737, 0.2664, 0.2791))
        # transform = DeNormalize((0.0, 0.0, 0.0),
        #                         (1.0, 1.0, 1.0))
        return transform
...
if __name__ == '__main__':
    ds = WebVision(base_path() + 'WebVision', train=True, task=-1)
    all_vectors = []
    for i in tqdm(range(len(ds))):
        ret = ds[i][1]
        all_vectors.append(ret)
        pass
    all_vectors = torch.stack(all_vectors, 0)
    pass