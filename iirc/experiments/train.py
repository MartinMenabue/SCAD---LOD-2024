...
def epoch_train(model, dataloader, config, metadata, gpu=None, rank=0, epoch=0, progress_bar=None):
    train_loss = 0
    ...
    for i, minibatch in enumerate(dataloader):
        ...
        in_buffer = None   # aggiunto da me

        predictions, loss = model.observe(images, labels, in_buffer, train=True, epoch=epoch, not_aug_inputs=minibatch[3])
        ...
        progress_bar.prog(i, len(dataloader), epoch, config['cur_task_id'], loss)
...
def evaluate(model, dataloader, config, metadata, test_mode=False, gpu=None):
    ...
    with torch.no_grad():
        for idx, minibatch in enumerate(dataloader):
            if config['debug_mode'] and idx > 3:
                break
            labels_names = list(zip(minibatch[1], minibatch[2]))
            labels = transform_labels_names_to_vector(
                labels_names, num_seen_classes, class_names_to_idx
...
def task_train(model, buffer, lifelong_datasets, config, metadata, logbook, dist_args=None, progress_bar=None):
    ...
    if distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            task_train_data, num_replicas=dist_args["world_size"], rank=rank)
    ...
    train_loader = data.DataLoader(
        ...
        task_train_data, batch_size=config["batch_size"], shuffle=(train_sampler is None),
    ...
    for epoch in range(config['task_epoch'], num_epochs):
        ...
        log_dict = {}
        train_loss, train_metrics = epoch_train(model, train_loader, config, metadata, gpu, rank, epoch, progress_bar)
...
def tasks_eval(model, dataset, cur_task_id, config, metadata, logbook, dataset_type="valid", dist_args=None):
    ...
    metrics_dict = {}
    model.net.eval()
    for task_id in tqdm(range(cur_task_id + 1)):
    ...
    print(metrics_dict, file=sys.stderr)
    # if config['wandb_log']:
    #     wandb.log(metrics_dict)

    # if rank == 0:
    #     #utils.log_task(cur_task_id, metrics_dict, logbook)      # qui viene loggato il metrics_dict su wandb
    #     if distributed:
    #         dist.barrier()
