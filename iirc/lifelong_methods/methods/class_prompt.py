...
from lifelong_methods.methods.class_prompt_utils.vit_prompt import vit_base_patch16_224_class_prompt
...
class Model(BaseMethod):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int], config: Dict):
        ...
        self.net = vit_base_patch16_224_class_prompt(pretrained=True, num_classes=self.num_classes)
        self.current_task = 0
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        loss = self.bce(logits / self.temperature, target)
