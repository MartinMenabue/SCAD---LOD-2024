...
from lifelong_methods.methods.scad_utils.afd import DiverseLoss, TestLoss, ChannelDiverseLoss, DiverseLossFixDiagonal, DiverseLossFixNormalized, DiverseLossFixNormalizedDiagonal
from lifelong_methods.methods.scad_utils.afd import Normalize
from lifelong_methods.methods.scad_utils.afd import TeacherTransform
from lifelong_methods.methods.scad_utils.afd import TeacherForcingLoss

from lifelong_methods.methods.scad_utils.adapters import MixerAttention
from lifelong_methods.methods.scad_utils.adapters import ChannelAttention
from lifelong_methods.methods.scad_utils.adapters import DoubleAttention
from lifelong_methods.methods.scad_utils.adapters import MockAttention
from lifelong_methods.methods.scad_utils.adapters import MimickingAttention
from lifelong_methods.methods.scad_utils.adapters import DoubleAttentionViT
from lifelong_methods.methods.scad_utils.adapters import TransformerAttention, TransformerAttentionLayerNorm, AttentionProbeCls, AttentionProbeClsNorm
from lifelong_methods.methods.scad_utils.adapters import ClipCrossAttention,TransformerAttentionClip, TransformerAttentionProj, TaT, TaTV2, TaTNorm
from lifelong_methods.methods.scad_utils.adapters import AttentionProbeClsNoGumbel, AttentionProbeClsNormNoGumbel, AttentionProbeNew, AttentionProbeFilter, AttentionProbeSoftmax, AttentionProbeV2
from lifelong_methods.methods.scad_utils.adapters import AttentionProbeV2Softmax

from lifelong_methods.methods.scad_utils.adapters_v2 import ClipCrossAttentionV2
from lifelong_methods.methods.scad_utils.adapters_v2 import ClipCrossAttentionV3, ClipCrossAttentionV4, ClipCrossAttentionV5, ClipCrossAttentionV6, ClipCrossAttentionV7
...
class TokenizedDistillation(nn.Module):
    ...
    def __init__(self, input_shape: Tuple[int], n_tasks: int,
                 n_cla_per_tsk: int, adatype: str = 'mixer',
        ...
        self.num_classes = sum(self.n_cla_per_tsk)
    ...
    def build_attn_module(self):
        if self.adatype == 'attention_probe_v2_softmax':
            return AttentionProbeV2Softmax(self.embed_dim, self.n_tasks, self.use_conditioning)
        if self.adatype == 'attention_probe_v2':
            return AttentionProbeV2(self.embed_dim, self.n_tasks, self.use_conditioning)
        if self.adatype == 'attention_probe_softmax':
            return AttentionProbeSoftmax(self.embed_dim, self.n_tasks, self.use_conditioning)
        if self.adatype == 'attention_probe_filter':
            return AttentionProbeFilter(self.embed_dim, self.n_tasks, self.use_conditioning)
        if self.adatype == 'attention_probe_new':
            return AttentionProbeNew(self.embed_dim, self.n_tasks, self.use_conditioning)
    ...
    def forward(self, fm_s, fm_t, targets, teacher_forcing, attention_map, task_labels):
        ...
        if ('ablation_type' not in os.environ or os.environ['ablation_type'] != 'no_diverse') and \
                    type(self.attn_fn) not in [MockAttention, AttentionProbeNew]:
