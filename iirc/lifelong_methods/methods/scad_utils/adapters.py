...
from lifelong_methods.methods.scad_utils.afd import BinaryGumbelSoftmax
from lifelong_methods.methods.scad_utils.afd import HardAttentionSoftmax

from lifelong_methods.methods.scad_utils.utils import ConditionalLinear
from lifelong_methods.methods.scad_utils.utils import WrapperNOTConditionalLinear
from lifelong_methods.methods.scad_utils.utils import TaskPrompter
from lifelong_methods.methods.scad_utils.utils import MLPMixer
from lifelong_methods.methods.scad_utils.utils import PiecewiseRect
from lifelong_methods.methods.scad_utils.utils import MLPMixerWithBottleneck
from lifelong_methods.methods.scad_utils.utils import ConVitWithBottleneck
from lifelong_methods.methods.scad_utils.utils import MyGumbelSoftmax
import math
import torch.nn.functional as F

from lifelong_methods.methods.scad_utils.afd import ConditionalLinear as ConditionalLinearOriginal
from lifelong_methods.methods.scad_utils.afd import ConditionalBatchNorm2d
...
class MixerAttention(nn.Module):
    ...
    def compute_distance(self, fm_s, fm_t, rho):
        fm_s, fm_t = self.norm(fm_s), self.norm(fm_t)
        dist = (fm_s - fm_t) ** 2
        dist = rho * dist
...
class AttentionProbeClsNorm(nn.Module):
    ...
    def compute_distance(self, fm_s, fm_t, rho):
        fm_s_norm = F.normalize(fm_s, dim=-1, p=2)
        fm_t_norm = F.normalize(fm_t, dim=-1, p=2)
        x_s = torch.bmm(fm_s_norm, fm_s_norm.permute(0, 2, 1))
        x_t = torch.bmm(fm_t_norm, fm_t_norm.permute(0, 2, 1))
        x_s = x_s[:, 0, 1:]
        x_t = x_t[:, 0, 1:]
        dist = (x_s - x_t) ** 2
        rho = rho.squeeze(1)
...
class AttentionProbeNew(nn.Module):
    def __init__(self, embed_dim, n_tasks, use_conditioning, seq_len=197) -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.n_tasks = n_tasks
...
class AttentionProbeFilter(nn.Module):
    def __init__(self, embed_dim, n_tasks, use_conditioning, seq_len=197) -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.n_tasks = n_tasks
        ...
        self.projections = nn.ModuleDict({str(k): nn.Linear(self.seq_len-1, self.seq_len-1) for k in range(self.n_tasks)})

    def forward(self, fm_t: torch.Tensor, y: torch.Tensor, tasks_id: torch.Tensor):
        norm = torch.norm(fm_t, dim=-1, keepdim=True)
        fm_t = torch.div(fm_t, norm + 1e-8)
        x = torch.bmm(fm_t, fm_t.permute(0, 2, 1))
        x = x[:, 0, 1:]
        ret = [self.projections[str(tid)](inp) for tid, inp in zip(tasks_id.tolist(), x)]
        rho = torch.stack(ret, dim=0)
        logits = torch.zeros_like(fm_t).unsqueeze(2).repeat(1, 1, 2, 1)
        return rho, logits
    ...
    def compute_distance(self, fm_s, fm_t, rho):
        norm = torch.norm(fm_t, dim=-1, keepdim=True)
        fm_t = torch.div(fm_t, norm + 1e-8)
        norm = torch.norm(fm_s, dim=-1, keepdim=True)
        fm_s = torch.div(fm_s, norm + 1e-8)
        x_s = torch.bmm(fm_s, fm_s.permute(0, 2, 1))
        x_t = torch.bmm(fm_t, fm_t.permute(0, 2, 1))
        x_s = x_s[:, 0, 1:]
        x_t = x_t[:, 0, 1:]
...
class AttentionProbeSoftmax(nn.Module):
    def __init__(self, embed_dim, n_tasks, use_conditioning, seq_len=197, temp=0.4) -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.n_tasks = n_tasks
        self.use_conditioning = use_conditioning
    ...
    def compute_distance(self, fm_s, fm_t, rho):
        ...
        x_s = x_s[:, 0, 1:]
        x_t = x_t[:, 0, 1:]
        w = torch.softmax(x_t / self.temp, dim=-1)
        x_t = x_t * w
        x_s = x_s * w
        dist = (x_s - x_t) ** 2
        dist = dist.mean(1)
...
class AttentionProbeV2(nn.Module):
    def __init__(self, embed_dim, n_tasks, use_conditioning, seq_len=197, temp=0.4) -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.n_tasks = n_tasks
    ...
    def compute_distance(self, fm_s, fm_t, rho):
        ...
        x_s = x_s[:, 0, 1:]
        x_t = x_t[:, 0, 1:]
        dist = ((x_s - x_t) ** 2).sum(1).mean()
        return dist

class AttentionProbeV2Softmax(nn.Module):
    def __init__(self, embed_dim, n_tasks, use_conditioning, seq_len=197, temp=1.0) -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.n_tasks = n_tasks
        self.use_conditioning = use_conditioning
