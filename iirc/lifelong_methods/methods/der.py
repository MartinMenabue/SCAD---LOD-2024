...
class Model(BaseMethod):
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        loss = self.bce(filtered_output / self.temperature, target)
        self.seen_y_so_far[:offset_2] |= y.any(dim=0).data
        
        loss_der = torch.tensor(0.0).to(self.device)
        if not self.buffer.is_empty():
            buf_inputs, buf_logits, buf_logits_mask = self.buffer.get_data(
        ...
        if train:
            ...
            if output_mask.sum() > 0:
                self.buffer.add_data(examples=not_aug_inputs, logits=output.data, logits_mask=output_mask.data)
            
            if self.config['wandb_log']:
                wandb.log({"loss": loss.item(), "loss_der": loss_der.item()})

        predictions = filtered_output > 0.0
