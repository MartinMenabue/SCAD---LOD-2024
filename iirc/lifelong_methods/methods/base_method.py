...
class BaseMethod(ABC, nn.Module):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int], config: Dict):
        ...
        self.num_tasks = len(self.n_cla_per_tsk)
    ...
    def save_checkpoints(self):
        ...
        j = self.config['conf_jobnum']
        print("Saving checkpoint into", f'{checkpoint_path}/{self.config["method"]}_{t}_{j}.pt', file=sys.stderr)
        torch.save(self.net.state_dict(), f'{checkpoint_path}/{self.config["method"]}_{t}_{j}.pt')
        with open(f'{checkpoint_path}/{self.config["method"]}_args_{j}.pkl', 'wb') as f:
            pickle.dump(self.config, f)
        with open(f'{checkpoint_path}/{self.config["method"]}_args_{j}.txt', 'wt') as f:
    ...
    def get_test_transforms(self, dataset_name):
        if "cifar100" in dataset_name:
            transform = torchvision.transforms.Compose([
                torchvision.transforms.ToPILImage(),
                torchvision.transforms.Resize(224),
