...
class Model(BaseMethod):
    ...
    def get_optimizer(self):
        classifier = self.net.fc if 'resnet' in self.config['network'] else self.net.head
        if self.config['optimizer'] == 'sgd':
            opt = torch.optim.SGD(classifier.parameters(), lr=self.lr, weight_decay=self.config['weight_decay'])
        elif self.config['optimizer'] == 'adam':
            opt = torch.optim.Adam(classifier.parameters(), lr=self.lr, weight_decay=self.config['weight_decay'])
        else:
