...
import timm
import wandb
from lifelong_methods.methods.krt_utils.model import Model as ModelBase
...
class Model(BaseMethod):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int], config: Dict):
        ...
        self.config['num_protos'] = 5
        self.config['sample_method'] = 'herding'
        self.config['kd_loss'] = 'pod_spatial'
        self.config['lambda_c'] = 0
        self.config['lambda_f'] = 180
        args = Namespace(**config)
        self.net = ModelBase(args, device=self.device)
        self.current_task = 0
    ...
    def _prepare_model_for_new_task(self, **kwargs) -> None:
        ...
        lifelong_datasets = kwargs['lifelong_datasets']
        self.net.before_task(lifelong_datasets, self.current_task, offset_1, offset_2)
        self.net.to(self.device)
        self.opt, self.custom_scheduler, self.loss = self.net.get_train_vars(lifelong_datasets['train'])
        pass
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        outputs = self.net(x)
        old_output = self.net.get_old_output(x)
        loss, cls_loss, pod_spatial, pod_flat = self.net.compute_loss(outputs, target, old_output)
        ...
        if train:
            ...
            if self.config['wandb_log']:
                log_dict = {
                    ...
                    'pod_spatial': pod_spatial.item(),
                    'pod_flat': pod_flat.item(),
                    'lr': self.custom_scheduler.get_last_lr()[0],
                    'loss': loss.item()
                }
                wandb.log(log_dict)

        predictions = outputs['logits'] > 0.0
