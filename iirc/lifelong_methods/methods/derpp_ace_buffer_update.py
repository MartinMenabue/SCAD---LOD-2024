...
class Model(BaseMethod):
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        if not self.buffer.is_empty():
            buf_inputs_1, _, buf_logits_1, buf_task_labels, buf_logits_mask_1 = self.buffer.get_data(
    ...
    def consolidate_task_knowledge(self, **kwargs) -> None:
        ...
        with torch.no_grad():
            ...
            cur_task_mask = torch.zeros(self.num_classes, dtype=bool).to(self.device)
            cur_task_mask[offset_1:offset_2] = True


            # if chosen.any() and self.current_task == 1:
            #TODO: gestisci update logits
            to_transplant = self.update_logits(buf_logits, buf_outputs, buf_labels, cur_task_mask, self.current_task, self.num_tasks - self.current_task)
            self.buffer.logits[buf_idx, :] = to_transplant.to(self.buffer.device)
            self.buffer.task_labels[buf_idx] = torch.full((len(buf_idx),), self.current_task, dtype=torch.long).to(self.buffer.device)
            new_logits_mask = torch.zeros((len(buf_idx), self.num_classes), dtype=bool).to(self.buffer.device)
            new_logits_mask[:, :offset_2] = True
            self.buffer.logits_mask[buf_idx, :] = new_logits_mask.to(self.buffer.device)

        self.current_task += 1

        pass

    def update_logits(self, old, new, gt, cur_task_mask, task_start, n_tasks=1):
        #transplant = new[:, task_start * self.cpt:(task_start + n_tasks) * self.cpt]
        #gt_values = old[torch.arange(len(gt)), gt]
        transplant = new[:, cur_task_mask]
        gt_max_values = []
        for i, el in enumerate(gt):
            gt_max_values.append(old[i, el.bool()].max())
        gt_max_values = torch.stack(gt_max_values)
        
        max_values = new[:, cur_task_mask].max(1).values

        coeff = self.config['xder_gamma'] * gt_max_values / max_values
        coeff = coeff.unsqueeze(1).repeat(1, cur_task_mask.sum())
        mask = (max_values > gt_max_values).unsqueeze(1).repeat(1, cur_task_mask.sum())
        transplant[mask] *= coeff[mask]
        old[:, cur_task_mask] = transplant
        return old
