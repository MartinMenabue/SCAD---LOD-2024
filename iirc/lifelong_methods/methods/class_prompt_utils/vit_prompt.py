...
class Prompt(nn.Module):
    def __init__(self, num_classes, n_tokens, embed_dim) -> None:
    ...
    def get_prompts(self, layer, labels):
        if labels is None:
            return None
        p = getattr(self, f'e_p_{layer}', None)
        if p is None:
            return None
        return p[labels]
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.prompt = Prompt(self.num_classes, 2, embed_dim)
    ...
    def forward_features(self, x, labels=None):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for i, blk in enumerate(self.blocks):
                p = self.prompt.get_prompt(i, labels)
                x = blk(x, prompt=p)
...
@register_model
def vit_base_patch16_224_class_prompt(pretrained=False, **kwargs):
