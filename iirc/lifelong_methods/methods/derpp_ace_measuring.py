...
class Model(BaseMethod):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int], config: Dict):
        ...
        self.teacher = timm.create_model(self.config['network'], pretrained=True, num_classes=self.num_classes)
        self.teacher.eval()
        for p in self.teacher.parameters():
            p.requires_grad = False
        self.hh_t = HooksHandlerViT(self.teacher)
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        with torch.no_grad():
            outputs_t = self.teacher(x)
            logits_t = outputs_t['output']
            block_outputs_t = outputs_t['block_outputs']
            fm_t = block_outputs_t[-1]
            fm_t = F.normalize(fm_t, p=2, dim=-1)
            attn_map_t = torch.bmm(fm_t, fm_t.permute(0, 2, 1))
            attn_probe_t = attn_map_t[:, 0, 1:]
        outputs_s = self.net(x)
        output = outputs_s['output']
        block_outputs_s = outputs_s['block_outputs']
        with torch.no_grad():
            fm_s = block_outputs_s[-1]
            fm_s = F.normalize(fm_s, p=2, dim=-1)
            attn_map_s = torch.bmm(fm_s, fm_s.permute(0, 2, 1))
            attn_probe_s = attn_map_s[:, 0, 1:]
            loss_dist = ((attn_probe_t - attn_probe_s) ** 2).sum(1).mean()
        ...
        if not self.buffer.is_empty():
            ...
            buf_outputs = buf_outputs['output']
            ...
            buf_outputs = self.net(buf_inputs)
            buf_outputs = buf_outputs['output']
            loss_re = self.bce(buf_outputs[:, :offset_2], buf_labels[:, :offset_2].float())
        ...
        if train:
            ...
            if self.config['wandb_log']:
                wandb.log({"loss": loss.item(), "loss_der": loss_der.item(), "loss_re": loss_re.item(), 'loss_dist': loss_dist.item()})
