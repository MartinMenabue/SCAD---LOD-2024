...
class Model(BaseMethod):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int], config: Dict):
        ...
        self.config['xder_gamma'] = self.config['xder_gamma'] or 0.85
        self.logit_threshold = np.log((0.5 * self.config['xder_gamma'])/(1-(0.5 * self.config['xder_gamma'])))
    ...
    def update_logits(self, old, new, gt, task_start, n_tasks=1):
        ...
        transplant = new[:, start_past_classes:start_past_classes+tot_past_classes]
        ...
        if self.config['rescale_logits_update']:
            ...
            transplant = ((transplant-transplant.min())/(transplant.max()-transplant.min()))*(self.logit_threshold - transplant.min())+transplant.min()

        old[:, start_past_classes:start_past_classes+tot_past_classes] = transplant
    ...
    def update_buffer(self, buf_idx, buf_inputs, buf_labels, buf_logits, buf_outputs, buf_tl):
        ...
        if chosen.any():
            ...
            to_transplant = self.update_logits(buf_logits[chosen], buf_outputs[chosen], buf_labels[chosen], self.current_task, self.num_tasks - self.current_task)
            self.buffer.logits[buf_idx[chosen]] = to_transplant.to(self.buffer.device)
            offset_1 = sum(self.n_cla_per_tsk[:self.current_task])
            offset_2 = sum(self.n_cla_per_tsk[:self.current_task + 1])
            self.buffer.logits_mask[buf_idx[chosen], offset_1:offset_2] = True
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        if not self.buffer.is_empty():
            # ----- Distillation -----
            buf_index_mse, buf_inputs_mse, buf_labels_mse, buf_logits_mse, buf_task_labels_mse, buf_logits_mask_mse = self.buffer.get_data(
            ...
            buf_logits_mse_modified = buf_logits_mse.masked_fill(~buf_logits_mask_mse, 0.0)
            buf_outputs_mse_modified = buf_outputs_mse.masked_fill(~buf_logits_mask_mse, 0.0)
            # buf_logits_mse[~buf_logits_mask_mse] = 0.0
            # buf_outputs_mse[~buf_logits_mask_mse] = 0.0
            loss_der = F.mse_loss(buf_outputs_mse_modified, buf_logits_mse_modified)
            loss += self.config['der_alpha'] * loss_der

            # ----- BCE Replay -----
            buf_index_ce, buf_inputs_ce, buf_labels_ce, buf_logits_ce, buf_task_labels_bce, buf_logits_mask_ce = self.buffer.get_data(
                self.config['batch_size'], transform=self.transform, return_index=True)
            
            buf_outputs_ce = self.net(buf_inputs_ce)
            loss_re += self.bce(buf_outputs_ce[:, :offset_2], buf_labels_ce[:, :offset_2].float())
            loss += self.config['der_beta'] * loss_re

            self.update_buffer(
                (buf_index_mse, buf_index_ce),
                (buf_inputs_mse, buf_inputs_ce),
                (buf_labels_mse, buf_labels_ce),
                (buf_logits_mse, buf_logits_ce),
                (buf_outputs_mse, buf_outputs_ce),
                (buf_task_labels_mse, buf_task_labels_bce),
            )
